...
(function(){
  ...
  */
  ...
  var analyzedCols = [];
  var allColumns = []; // tadeu
  var previousOp = new Array();
  
 // tadeu
  function fillJSONData(problem) {
	  var data = problem.options;
	  
	  $("#json--alldata").append("<table style=\"padding: 3px\">");
	  $("#json--alldata").append("<td>Nome</td><td>Rentabilidade</td><td>Risco</td><td>Taxa</td><td>Contribuição</td><td>Aporte</td><td>Tempo de Contribuição</td>");
	  data.forEach(function (one) {
		  $("#json--alldata").append('<tr>');
		  $("#json--alldata").append('<td>' + one.name + '</td>' );
		  $("#json--alldata").append('<td style=\"padding: 10px\">' + one.values.rentabilidade + '%</td>' );
		  $("#json--alldata").append('<td style=\"padding: 10px\">' + one.values.risco+ '</td>' );
		  $("#json--alldata").append('<td style=\"padding: 10px\">' + one.values.taxas + '%</td>' );
		  $("#json--alldata").append('<td style=\"padding: 10px\">R$' + one.values.contribuicao + '</td>' );
		  $("#json--alldata").append('<td style=\"padding: 10px\">R$' + one.values.aporte + '</td>' );
		  $("#json--alldata").append('<td style=\"padding: 10px\">' + one.values.tempocontribuicao + ' anos</td>' );
		  $("#json--alldata").append('</tr>');
	  });
	  
	  $("#json--alldata").append("</table>");
  ...
  function analyze() {
    ...
    var columns1 = theProblem.columns; //tadeu
    
    dilemma(theProblem)
      .then(function(result){
        analyzedCols = selectedColumns; 
        allColumns = columns1; //tadeu
  ...
  $(document).ready(function () {
    ...
    $('#ver-conjunto-dados').click(function() {
    	console.log("Estou aqui na funcao");
    	$('#json--alldata').show();
    });
